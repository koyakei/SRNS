...
$tagSSugID = 2143;//検索時の関連性サジェストのタグ
...
print_r ($tagA);//ここに複数のタグを入れる
	if ($tagA == null) {
			...
			$From = $eachTag[ID];
			$sql = "SELECT  `tagLink`.`LFrom` AS TLFROM, `article` . * FROM  `LINK` INNER JOIN  `LINK` AS tagLink ON  `LINK`.`ID` = `tagLink`.`LTo`, `article`  WHERE  `LINK`.`LFrom` =$article[ID] AND `tagLink`.`LFrom` =$tagSSugID  AND `article` . `ID` = `LINK` . `LTo`;"; //親タグと当該タグのリンク重複を調べる
